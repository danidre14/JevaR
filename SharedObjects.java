...
public class SharedObjects {
    /* Usage:
     * In your main file, initialize a shared object / bucket with:
     * SharedObjects so = SharedObjects.getLocal("my_so"); // you can name each bucket whatever you like
     * 
     * To add a key-value pair:
     * so.setItem("key1", "value");
     * so.setItem("key2", 123); // it stores the .toString() of any <value> you send to it
     * 
     * To get a key-value pair:
     * String key1 = so.getItem("key1"); // returns string versions
     * int key2 = Integer.parseInt(so.getItem("key2")); // convert to your intended datatype
     * 
     * To remove a key-value pair from bucket:
     * so.removeItem("key1");
     * so.removeItem("key2");
     * 
     * Don't worry about the inner workings. The text files are created and deleted for you by default handled by the class itself.
     */
    private String name;
    private String path;
    private File file;
    ...
    private static HashMap<String, SharedObjects> sharedObjects;

    static {
        dir = "./storage/";
        sharedObjects = new HashMap<String, SharedObjects>();
    }

    public static SharedObjects getLocal(String sharedObjectName) {
        SharedObjects so = sharedObjects.get(sharedObjectName);
        if (so != null)
            return so;

        so = new SharedObjects(sharedObjectName);
        sharedObjects.put(sharedObjectName, so);
        return so;
    }

    private SharedObjects(String sharedObjectName) {
